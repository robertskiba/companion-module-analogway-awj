...
export default class Presets {
	...
	/**
	 * All presets as generated by the getters named in presetsToUse, still carrying their grouping category
	 */
	private get categorizedPresets(): CategorizedPresetDefinitions {
		return this.presetsToUse.reduce(
	 ...
	 */
	get allPresets(): CompanionPresetDefinitions {
		const presetDefinitions: CompanionPresetDefinitions = {}
		for (const [id, preset] of Object.entries(this.categorizedPresets)) {
			const { category: _category, ...rest } = preset
			presetDefinitions[id] = rest
		}
		return presetDefinitions
	}

	/**
	 * The grouping structure for the preset browser, derived from each preset's category
	 */
	get presetStructure(): CompanionPresetSection[] {
		const sections = new Map<string, string[]>()
		for (const [id, preset] of Object.entries(this.categorizedPresets)) {
			const ids = sections.get(preset.category) ?? []
			ids.push(id)
	...
	get multiviewerMemories() {
		...
		for (const multiviewer of this.choices.getMultiviewerArray()) {
			for (const memory of this.choices.getMultiviewerMemoryArray()) {
				...
				const nameSuffix = multimulti ? ' on Multiviewer ' + multiviewer : ''
