...
One thing worth knowing: **tally variables aren't generated for every possible combination up front.** They only appear once you actually place a matching "Source Tally" feedback on a button - at that point the variable is added and can also be used in triggers. Want a variable for Input 3 on screen S2 preview? Add a Source Tally feedback checking exactly that first, and it shows up as `S2.prw.tally.IN3`. Each placed feedback owns exactly one variable: change its Screens, Preset or Source and the variable is renamed accordingly, with the previous name dropped rather than left behind.
...
- `SelectedLayer.Input.Number` / `.Input.Name` / `.Input.width` / `.Input.height` - the source assigned to the first currently selected Layer. These stay blank when there is nothing to report - unlike a `.source` variable, which says `NONE` for an empty layer.
- `SelectedLayer.Crop.Top` / `.Bottom` / `.Left` / `.Right` / `.Mask.Top` / `.Bottom` / `.Left` / `.Right` - crop/mask of the first currently selected Layer.
- `Sx.pgm.layery.source` / `.status` / `.width` / `.height` / `.x` / `.y` - a Layer's own source, live signal status, size and position, for currently existing layers only (example: `S3.pgm.layer2.source` shows the source of Layer 2 on Screen 3's Program). Every one of these exists for both banks: use `.prw.` instead of `.pgm.` to read what is lined up on Preview. `Sx.pgm.layerbg.source` covers the background layer, and on Alta 4K / Midra 4K `Sx.pgm.layerfg.source` gives the Foreground Image slot (1-4). An empty layer reports `NONE` rather than a blank value, so an expression can test for it explicitly.
- `Sx.pgm.tally.INy` - the tally state (0 or 1) of a given source in a given screen's program/preview (example: `S2.pgm.tally.IN3` shows the tally state of Input 3 in Screen 2's program - see the caveat on tally variables in [About Variables](#variables)). The preset segment is `pgm` or `prw`, or `bothand`/`bothor` for the two "Both" choices. If the feedback's Screens field is left on "Any Screen", "Selected Screens" or "First/Only Selected Screen", the name keeps that choice instead of the screens it currently resolves to - `ALL.pgm.tally.IN3`, `SEL.pgm.tally.IN3`, `FIRSTSEL.pgm.tally.IN3` - so it stays stable when your selection changes. Only an explicitly named list is spelled out, as in `S1-S2-A1.pgm.tally.IN3`.
