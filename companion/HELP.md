...
- `Sx.pgm.layery.source` / `.status` / `.width` / `.height` / `.x` / `.y` - a Layer's own source, live signal status, size and position, for currently existing layers only (example: `S3.pgm.layer2.source` shows the source of Layer 2 on Screen 3's Program). Every one of these exists for both banks: use `.prw.` instead of `.pgm.` to read what is lined up on Preview. `Sx.pgm.layerbg.source` covers the background layer, and on Alta 4K / Midra 4K `Sx.pgm.layerfg.source` gives the Foreground Image slot (1-4, blank if none).
