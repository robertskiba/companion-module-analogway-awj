...
- [ ] "Toggle" choice on every on/off flag. Transitions' Allow Cross Effect/Depth are done (2026-09-22) - the interesting half, since Midra encodes them as a single negative `DISABLE_*` token whose presence means "off", so a toggle has to read the state inverted before flipping it. Speed Linear does not exist on Midra. **Open: Effects and Border.**
