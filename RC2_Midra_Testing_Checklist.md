...
- [ ] Save/Revert Screen Memory Changes - still not registered for Midra. It reverts a *loaded* memory to its saved state, a different command from the plain save above, and it has not been observed on the wire here. Decide: capture and enable, or leave as a permanent LivePremier4-only feature and say so in the docs.
